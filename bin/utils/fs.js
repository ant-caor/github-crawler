...
const readPRsDir = (dir = "prs") => {
  ...
  fs.readdirSync(dir).forEach((owner) => {
    if (fs.lstatSync(`${dir}/${owner}`).isDirectory()) {
      fs.readdirSync(`${dir}/${owner}`).forEach((repo) => {
        if (fs.lstatSync(`${dir}/${owner}/${repo}`).isDirectory()) {
          fs.readdirSync(`${dir}/${owner}/${repo}`).forEach((pr_file) => {
            const content = fs.readFileSync(
              `${dir}/${owner}/${repo}/${pr_file}`,
              {
                encoding: "utf-8",
              }
            );
            ...
            console.log(
              `This pr has: ${pr.issueComments.length} issue comments`
            );
